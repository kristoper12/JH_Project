================================================================================
    MFC 라이브러리 : MFC_SDI_PREPROCESS 프로젝트 개요
===============================================================================

응용 프로그램 마법사에서 이 MFC_SDI_PREPROCESS 응용 프로그램을 만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 프로그램을 작성하기 위한 기본 구조를 제공합니다.

MFC_SDI_PREPROCESS 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.

MFC_SDI_PREPROCESS.vcxproj
...
MFC_SDI_PREPROCESS.vcxproj.filters
...
MFC_SDI_PREPROCESS.h
    해당 응용 프로그램의 기본 헤더 파일입니다.
    여기에는 resource.h를 비롯한 다른 프로젝트별 헤더가 포함되어 있으며 CMFC_SDI_PREPROCESSApp 응용 프로그램 클래스가 선언되어 있습니다.

MFC_SDI_PREPROCESS.cpp
    이는 응용 프로그램 클래스 CMFC_SDI_PREPROCESSApp가 포함된 기본 응용 프로그램 소스 파일입니다.

MFC_SDI_PREPROCESS.rc
...
res\MFC_SDI_PREPROCESS.ico
    아이콘 파일이며, 응용 프로그램의 아이콘으로 사용됩니다. 이 아이콘은 기본 리소스 파일인 MFC_SDI_PREPROCESS.rc에 의해 포함됩니다.

res\MFC_SDI_PREPROCESS.rc2
...
MFC_SDI_PREPROCESSDoc.h, MFC_SDI_PREPROCESSDoc.cpp - 문서
    이 파일에는 CMFC_SDI_PREPROCESSDoc 클래스가 포함됩니다. 이 파일을 편집하여 특수 문서 데이터를 추가하고 CMFC_SDI_PREPROCESSDoc::Serialize를 통해 파일 저장 및 로드를 구현합니다.

MFC_SDI_PREPROCESSView.h, MFC_SDI_PREPROCESSView.cpp - 문서 뷰
    이 파일에는 CMFC_SDI_PREPROCESSView 클래스가 포함됩니다.
    CMFC_SDI_PREPROCESSView 개체는 CMFC_SDI_PREPROCESSDoc 개체를 보는 데 사용됩니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더(PCH) 파일 MFC_SDI_PREPROCESS.pch와 미리 컴파일된 형식(PCT) 파일 StdAfx.obj를 빌드하는 데 사용됩니다.

Resource.h
    새 리소스 ID를 정의하는 표준 헤더 파일입니다. Microsoft Visual C++에서 이 파일을 읽고 업데이트합니다.

MFC_SDI_PREPROCESS.manifest
